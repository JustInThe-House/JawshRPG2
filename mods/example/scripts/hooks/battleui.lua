-- i think the only things changed here are the soul sprite sizes
...
function BattleUI:drawState()
    if Game.battle.state == "MENUSELECT" then
        ...
        Draw.draw(self.heart_sprite, 5 + ((Game.battle.current_menu_x - 1) * 230), 30 + ((Game.battle.current_menu_y - (page*3)) * 30),0,0.25,0.25)
    ...
    elseif Game.battle.state == "ENEMYSELECT" or Game.battle.state == "XACTENEMYSELECT" then
        ...
        Draw.setColor(Game.battle.encounter:getSoulColor())
        Draw.draw(self.heart_sprite, 55, 30 + ((Game.battle.current_menu_y - page_offset) * 30),0,0.25,0.25)
