...
function DarkConfigMenu:update()
    if self.state == "MAIN" then
        if Input.pressed("confirm") then
            ...
            elseif self.currently_selected == 3 then
                if MathUtils.round(Kristal.Config["fps"]) == 60 then
                    Kristal.Config["fps"] = 120
                else
                    Kristal.Config["fps"] = 60
                end
                FRAMERATE = Kristal.Config["fps"]
              --  self.state = "FPS"
               -- self.noise_timer = 0
            elseif self.currently_selected == 4 then
                Kristal.Config["showFPS"] = not Kristal.Config["showFPS"] --was ["simplifyVFX"]
    ...
    elseif self.state == "FPS" then
        if Input.pressed("cancel") or Input.pressed("confirm") then
            Kristal.Config["fps"] = MathUtils.round(Kristal.Config["fps"])
            FRAMERATE = MathUtils.round(FRAMERATE)
            self.ui_select:stop()
            self.ui_select:play()
            self.state = "MAIN"
            return
        ...
        self.noise_timer = self.noise_timer + DTMULT
        if Input.down("left") then
            Kristal.Config["fps"] = MathUtils.approach(Kristal.Config["fps"], 30, DTMULT)
            FRAMERATE = MathUtils.approach(FRAMERATE, 30, DTMULT)
            if self.noise_timer >= 3 then
                self.noise_timer = self.noise_timer - 3
                Assets.stopAndPlaySound("noise")
            end
        ...
        if Input.down("right") then
            Kristal.Config["fps"] = MathUtils.approach(Kristal.Config["fps"], 240, DTMULT)
            FRAMERATE = MathUtils.approach(FRAMERATE, 240, DTMULT)
            if self.noise_timer >= 3 then
                self.noise_timer = self.noise_timer - 3
                Assets.stopAndPlaySound("noise")
            end
...
function DarkConfigMenu:draw()
    ...
    if self.state ~= "CONTROLS" then
        ...
        love.graphics.print("Target FPS", 88, 38 + (2 * 32)) -- selected item 3
        Draw.setColor(PALETTE["world_text"])
        love.graphics.print("Show FPS", 88, 38 + (3 * 32)) -- was Simplify VFX
        love.graphics.print("Fullscreen", 88, 38 + (4 * 32))
        love.graphics.print("Auto-Run", 88, 38 + (5 * 32))
        love.graphics.print("Return to Title", 88, 38 + (6 * 32))
        ...
        love.graphics.print(MathUtils.round(Kristal.Config["fps"]), 348, 38 + (2 * 32))
        Draw.setColor(PALETTE["world_text"])
        love.graphics.print(Kristal.Config["showFPS"] and "ON" or "OFF", 348, 38 + (3 * 32)) --was ["simplifyVFX"]
        love.graphics.print(Kristal.Config["fullscreen"] and "ON" or "OFF", 348, 38 + (4 * 32))
        love.graphics.print(Kristal.Config["autoRun"] and "ON" or "OFF", 348, 38 + (5 * 32))

        Draw.setColor(Game:getSoulColor())
        Draw.draw(self.heart_sprite, 63, 48 + ((self.currently_selected - 1) * 32), 0, 0.25, 0.25)
    else
        ...
        if dualshock then
            Draw.draw(self.heart_sprite, -2, 34 + ((self.currently_selected - 1) * 29), 0, 0.25, 0.25)
        else
            Draw.draw(self.heart_sprite, -2, 34 + ((self.currently_selected - 1) * 28) + 2, 0, 0.25, 0.25)
        end
    end
