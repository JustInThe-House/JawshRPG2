...
function SaveMenu:init(...)
    super.init(self, ...)
    self.party_leader = Game.world.player.actor.id
end
    ...
    if self.state == "MAIN" then
        ...
        if Input.pressed("confirm") then
            ...
            elseif self.selected_x == 2 and self.selected_y == 2 then
                if #Game.party == 2 then --assumes there are only 2 people in party!
                    Input.clear("confirm")
                    local char = Game.world:getPartyCharacterInParty(self.party_leader)
                    local first_follower_char
                    if char then
                        if char == Game.world.player then
                            for _, party in ipairs(Game.party) do
                                if self.party_leader ~= party.id then
                    ...
                    Game:removePartyMember(self.party_leader)
                    Game:addPartyMember(self.party_leader)
                    if Game.world.player then
                        Game.world:spawnFollower(Game.party_data[self.party_leader]:getActor(), { party = self.party_leader })
                    else
                        local x, y = Game.world.camera:getPosition()
                        Game.world:spawnPlayer(x, y, Game.party_data[self.party_leader]:getActor(), self.party_leader)
                    end
                    

                    -- THIS IS WHERE I WILL SWAP PARTY LEADERS
                end
                self.party_leader = Game.world.player.actor.id
            end
        end
...
function SaveMenu:draw()
    ...
    if self.state == "MAIN" then
        ...
        Draw.setColor(PALETTE["world_text"])
        love.graphics.print("Swap Leader", 350, 260)

        -- Heart
        local heart_positions_x = { 142, 322 }
        local heart_positions_y = { 228, 270 }
        Draw.setColor(Game:getSoulColor())
        Draw.draw(self.heart_sprite, heart_positions_x[self.selected_x], heart_positions_y[self.selected_y], 0, 0.25,
                  0.25)
    elseif self.state == "SAVE" or self.state == "OVERWRITE" then
        ...
        if self.selected_y == 4 then
            Draw.setColor(Game:getSoulColor())
            Draw.draw(self.heart_sprite, 236, 402, 0, 0.25, 0.25)
    ...
    if self.state == "OVERWRITE" then
        ...
        if self.selected_x == 1 then
            Draw.setColor(Game:getSoulColor())
            Draw.draw(self.heart_sprite, 142, 332, 0, 0.25, 0.25)
        ...
        if self.selected_x == 2 then
            Draw.setColor(Game:getSoulColor())
            Draw.draw(self.heart_sprite, 322, 332, 0, 0.25, 0.25)
...
function SaveMenu:drawSaveFile(index, data, x, y, selected, header)
    ...
    elseif not data then
        ...
        if selected then
            Draw.setColor(Game:getSoulColor())
            Draw.draw(self.heart_sprite, x + 161, y + 30, 0, 0.25, 0.25)
        end
    else
        ...
        if selected and not header then
            Draw.setColor(Game:getSoulColor())
            Draw.draw(self.heart_sprite, x + 18, y + 14, 0, 0.25, 0.25)
        end
    end
    Draw.setColor(1, 1, 1)
end
