return {
    intro = function(cutscene, event)
        ...
        cutscene:wait(2)
        cutscene:text("* Meh,[wait:5] probably didn't mean anything.", "real_neutral")
        cutscene:wait(1)
        cutscene:text("* Ugh,[wait:5] what am I gonna do...", "real_neutral")
        cutscene:text("* How am I gonna make this year the year of Jawsh...", "real_neutral")
        cutscene:wait(1)
        cutscene:text("* Maybe X the everything app will give me some inspiration.", "real_happy")]]
    ...
    transition = function(cutscene, event)
        if not Game:getFlag("can_leave_home", false) then -- may not need this if im going to make a bunch of stuff invisble. but may need to for a setflag, idk
            local jawsh = cutscene:getCharacter("jawsh")
            ...
            if choice == 1 then
                ...
                cutscene:setSpeaker(jawsh)
                cutscene:wait(3)
                --[[cutscene:text("* Aaaand[wait:3] X is making me feel even worse.", "real_neutral")
                cutscene:text("* I love seeing beheadings in my feed.", "real_disgusted")
                cutscene:wait(1)
                cutscene:text("* ...wait,[wait:5] Toby Fox is trending?", "real_neutral")
                cutscene:text("* ...[wait:7]...[wait:7]he made another game?", "real_shocked")
                cutscene:text("* That's stupid and it's going to suck.", "real_neutral")
                cutscene:wait(2)
                cutscene:text("* ...[wait:5]hold on,[wait:5] this is a perfect opportunity!", "real_soy")
                cutscene:text("* If I can get my opinion on this game out first...", "real_happy")
                cutscene:text("* ...then I'll be famous on X and get a bunch of supporters!", "real_soy")
                cutscene:text("* Bloody brilliant!", "real_soy")
                cutscene:text("* Let's see what this game is about...", "real_happy")
                cutscene:wait(2)
                cutscene:text("* ...[wait:5]It's in VR???", "real_shocked")
                cutscene:text("* Didn't know you could do pixel art in VR...", "real_disgusted")
                cutscene:wait(2)
                cutscene:text("* Well,[wait:5] I guess it's time to bust this thing out.", "real_neutral")
                ...
                cutscene:text("* I never expected to use this again,[wait:5] especially after FNAF VR...",
                              "real_neutral")
                cutscene:wait(1)
                cutscene:text("* Here goes...", "real_neutral")
                cutscene:text("insert sprite with VR headset on", "some_portrait_idk")]]
                ...
                cutscene:text("* They turned me into an orange.", "neutral")
                cutscene:text("* Cool.", "happy")
                cutscene:text("* Let's go find what's bad about this game so I can complain about it.", "happy")
                cutscene:setSpeaker()
                cutscene:text(
                    "[wait:5](This game controls like Deltarune. [wait:7]If you don't know that already,[wait:5] then you're screwed.)")
