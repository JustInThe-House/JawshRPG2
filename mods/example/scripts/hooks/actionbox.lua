...
function ActionBox:createButtons()
    ...
    for i,btn in ipairs(btn_types) do
        if type(btn) == "string" then
            local button = ActionButton(btn, self.battler, math.floor(start_x + ((i - 1) * 35)) + 0.5, 21) -- this is the 5 action buttons in battle
            button.actbox = self
...
function ActionBox:update()
    ...
    if Game.battle.current_selecting == self.index then -- this is the movement of the entire box up/down after selecting/cancelling a move.
...
function ActionBox:drawActionBox() -- this has to deal with the bottom battle box. the part that doesnt move.
    if Game.battle.current_selecting == self.index then
...
function ActionBox:drawSelectionMatrix()
    ...
    if Game.battle.current_selecting == self.index then
        ...
        for i = 0, 11 do
            ...
            love.graphics.setLineWidth(2) -- this is the thickness of the moving lines when selecting.
