...
function RudeBusterBeam:init(red, x, y, tx, ty, after)
    ...
    self.timer_bonusdamage = 0
    self.pressed_bonusdamage = 0
    self.bonus_anim = false
end

function RudeBusterBeam:update()
    self.timer_bonusdamage = self.timer_bonusdamage + DT
    ...
    if Input.pressed("confirm") and not self.pressed then
        self.pressed = true
        self.pressed_bonusdamage = self.timer_bonusdamage
    end

    if Utils.dist(self.x, self.y, self.target_x, self.target_y) <= 40 then
        if self.after_func then
            self.after_func(self.pressed_bonusdamage/self.timer_bonusdamage)
            if self.pressed_bonusdamage/self.timer_bonusdamage > 0.8 then
